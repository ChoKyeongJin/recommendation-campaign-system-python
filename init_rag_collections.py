...
def rebuild_knowledge_base(
    ...
    dimension_catalog_path: Path | None = None,
    member_value_index_path: Path | None = None,
) -> dict[str, Any]:
    payload = build_payload(
        ...
        dimension_payload=(
            load_json(dimension_catalog_path) if dimension_catalog_path and dimension_catalog_path.exists() else None
        ),
        member_value_payload=(
            load_json(member_value_index_path)
            if member_value_index_path and member_value_index_path.exists()
            else None
        ),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--recreate", action="store_true", help="Delete and recreate the indexed collections before indexing.")
    parser.add_argument(
        "--include-user-collection",
        action="store_true",
        help=(
            "Also index the standalone campaign_user_rag_nodes collection. "
            "Off by default: the runtime only queries the knowledge collection, and the "
            "campaign/user nodes are already merged into it."
        ),
    )
...
def main() -> None:
    ...
    if not args.skip_knowledge_build and not args.validate_only:
        knowledge_build = rebuild_knowledge_base(
            ...
            dimension_catalog_path=args.dimension_catalog,
            member_value_index_path=args.member_value_index,
        )
    ...
    collections = []
    if args.include_user_collection:
        collections.append(
            index_collection(
                client=client,
