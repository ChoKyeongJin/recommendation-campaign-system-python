...
(에이전트 12), 그 검증이 플랜의 전제 여러 개를 실제로 뒤집었다. 아래는 **바뀐 전제**를 먼저,
그다음 무엇을 했는지, 마지막으로 함정을 적는다.

## 플랜의 전제 중 틀린 것 — 검증이 뒤집은 것들

| 플랜이 적은 것 | 실제 |
|---|---|
| P2: "라이브 스키마에서 `unsupported_operations`·`status` 를 제거하라" | **그 스키마는 LLM 에 전송되지 않는다.** 라이브 도구 스키마(`CAMPAIGN_QUERY_PLAN_V4_TOOL`)의 최상위 키는 5개뿐이고 `semantic_ir` 문자열이 페이로드에 없다(실측). `SEMANTIC_IR_LLM_JSON_SCHEMA` 는 **내부 조립 계약**이고, 노출 금지는 이미 `test_v4_tool_schema_expressibility.py` 가 강제한다. 모델의 실제 산출면은 `audience_requirement.issues[].code` 하나다 |
| P5: "미지원 종결을 걷고 legacy 빌더를 부른다" | **그대로는 no-op 이다.** canonical 이 미지원으로 끝날 때 legacy 빌더가 읽을 `query_plan` 슬롯은 구조적으로 비어 있고(V4 노출면에 `target_user` 자체가 없다), 빈 플랜으로 dispatch 를 태우면 20개 빌더 전부 None 을 돌려준다(실측) |
| P1: "`attribute` 필드에 카탈로그 파생 어휘를 결속하라" | **그대로 하면 `co_purchase` 가 깨진다.** 그 관계의 `attribute` 값은 이력 속성이 아니라 엔터티(`product`)이고, `FieldSpec.vocabulary` 는 discriminator 를 지원하지 않는다. 재현된 실패(`attribute='grade'`)는 어휘를 닫지 않고 **별칭 해석만으로** 해결된다 |
| P4: "`audience_catalog.json` 전체 심볼 52" | 어느 세는 방식으로도 재현되지 않는다. sources 10 + fields 22 + metrics 10 = **42** |
| R5: "거부된 SQL 도 사유도 남지 않는다" | 근거는 **사라지지 않았다** — `selected_candidate` JSONB 안에 온전히 있다. 비어 있는 것은 **이름 있는 컬럼**(`generated_sql`/`error_detail`)이라, 운영 조회로는 안 보였다 |

## 관통하는 발견 — 결함의 대부분은 "선언은 맞았는데 배선이 없다"

이번에 고친 것 중 넷이 같은 모양이었다. 카탈로그가 **이미 올바르게 선언**하고 있는데
로더가 그 필드를 읽지 않아 선언이 죽어 있었다.

| 죽어 있던 선언 | 결과 |
|---|---|
| `metrics[].aliases` (10/10 선언) | `member_grade.aliases` 에 `"grade"` 가 처음부터 있었는데, 라이브 모델이 낸 `grade` 는 `catalog_metric_unregistered` 로 죽었다 |
| `sources[].aliases` (10/10), `fields[].aliases` (22/22) | canonical 의 표면 어휘 32종이 통째로 소비 불가. 실행 자산 색인의 코퍼스 히트가 51/77 에 머물렀다(살린 뒤 **74/77**) |
| `data_coverage.monthly_attribute_snapshot` 의 `from`/`to` | 로더는 `available_from`/`complete_through` 만 읽었다. **적재 구간 201701 이 한 번도 적용된 적이 없다** — canonical 은 적재 밖 월을 무제한으로 받아들이고 있었다 |
| `signal_coverage.consent = expressible:false` | 물리 컬럼 4개(`APP_PUSH_YN`·`SMS_YN`·`EMAIL_YN`·`AGREE_YN`)가 legacy 에 다 있는데 canonical 은 "표현 불가"로 선언돼 있었다 |

세 번째는 이번 작업이 **드러낸** 것이지 만든 것이 아니다. 연산자 별칭을 고치자
`as_of` 가 canonical 로 넘어갔고, 그러자 적재 구간 판정이 없다는 사실이 튀어나왔다.

## 무엇을 했나

**P0 — 안전망.** ① 거부된 후보의 SQL·사유를 이름 있는 컬럼으로 **투영**한다(새 판정이 아니다).
사유 수집은 후보의 하위 검증 보고를 **구조로** 훑는다 — 보고 이름 목록을 손으로 들면 새 관문의
거부가 다시 조용해진다. ② `required_clauses` 5/77 → **77/77**. 선언 원칙은 "프롬프트가 요구하는
**물리 축마다 하나씩**"이고, 같은 축 안의 다중성(한 테이블에 창 두 개)은 문자열 포함으로 판별할 수
없으므로 적지 않았다 — 못 하는 것을 적으면 가드가 아니라 장식이 된다. ③ 실측 귀결을 커밋된
기준선으로 고정하고 러너에 `--baseline` 대조를 붙였다.

**P1 — 심볼 결속.** 낱말형↔기호 별칭 표가 **네 벌** 따로 있었고 그중 하나는 서로를 반박했다 —
스키마는 모델에게 `eq|gte|lte` 를 허용값으로 **제시**하는데 카탈로그 해석기는 `= != > >= < <=` 만
등록해, **모델이 스키마를 지키면 반드시 실패**했다. 표를 `event_ir` 이 기호 집합 바로 옆에서 단독
소유하게 하고 해석기가 그 표로 환원한다. 도메인 선언(`eq|gte|lte`)은 **뒤집지 않았다** — 뒤집으면
`compositional_targeting` 의 다월 경로가 죽고 얻는 것이 없다(검증이 잡아냈다).

**P2 — capability 판정 회수.** 강등 가드의 조건을 바꿨다: "다른 생산자가 노드를 냈는가"(모델이
미지원을 선언할 때는 노드를 내지 않으므로 **보호가 필요한 경우에만 정확히 발동하지 않는**
자기무력화 가드) → "**선언된 실행 자산 중 이 의미를 처리하는 것이 있는가**". 판정자는 신규
`execution_assets.py` 하나이고, 표면어는 선언된 레지스트리 4종에서 파생한다.

세 갈래로 종결한다. **자산 없음** → 정직한 미지원(단, `kind` 는 닫힌 코드이고 문장은 결정론).
**자산 있음 + 노드 있음** → 가설로 강등. **자산 있음 + 노드 없음** → `semantic_registry_gap`
("자산은 선언됐는데 그 축을 낼 생산자가 없다"). 셋째가 중요하다 — 그냥 강등하면 플랜이
'조건 0개인 resolved' 로 통과해 조용한 오답이 된다.

**P3 — 결핍을 리터럴 색인과 대조.** `missing_field_causes` 가 라이브 경로에서 구조적으로 항상
`[]` 였다(`empty_semantic_ir` 이 리터럴 `[]` 를 박았다). 그래서 **시스템이 이미 추출해 정규화까지
마친 값을 사용자에게 되물었다**(#3 '10%'). 원인 판정을 붙이고 `structurer` 의 손코딩 특례
(`argument=="period"` + 두 종류만 아는)를 그 원인 읽기로 교체했다.

판정의 조인 키는 **근거 구간**이지 인자 이름이 아니다. 모델의 `argument` 는 닫힌 어휘가 아니라
이름→종류 표는 곧 낡는다. 구간을 보는 것이 핵심이기도 하다 — 예전 특례는 스팬을 안 봐서
**다른 절의 '3개월' 때문에 진짜 결핍인 맨 '최근'까지** 재방출로 보냈다.

**P4 — 커버리지 패리티.** `tools/canonical_coverage_inventory.py` 가 legacy ⊖ canonical 물리 컬럼
차집합을 기계로 낸다(**57 → 53 컬럼 / 19축**). 기준선 + 하향 전용 래칫을 세웠다. 그리고 수신동의
4축을 **선언 5줄로 닫았다** — fields 4개 + value_domain 1개, **코드 변경 0**:

```sql
-- audience_catalog.json 에 항목을 넣기만 하면 나온다
(B.EMAIL_YN = 'Y') AND (B.SMS_YN = 'Y')
```

architecture §8 이 약속한 "새 회원 속성 축 추가 = 항목 1개"가 실제로 성립함을 보인 첫 사례다.
`MEMBER_STATE_CD` 는 **일부러 남겼다** — canonical 경로가 기본 정상회원 필터를 무조건 AND 로
붙이므로, 선언만 하면 '휴면 회원' 요청이 조용히 0명이 된다.

**P5 — 판정 코드의 어휘.** `plan_validation._collect_unsupported_operations` 가 `_marker` 로
자유 텍스트를 읽어 **모델이 쓴 한국어 문장을 이슈 코드로 승격**하고 있었다(실측 코드 `표현_불가`).
고정 코드로 바꿨다. 원안(라우팅 회수)은 슬롯 생산자가 없어 no-op 이므로 착수하지 않았다.

**+ 자기반박 주장은 종결이 아니라 재시도다.** 위 P2 를 착지시킨 뒤 라이브를 찔러 보다가
가장 단순한 프롬프트가 막히는 것을 봤다:

```
'여성 회원을 찾아줘'
 → unsupported_semantics / argument='subject.gender'
   "The compiler cannot represent direct profile field comparison in the current execution target."
```

`subject.gender` 는 **카탈로그 필드**다. 등록됐다는 것이 곧 표현할 수 있다는 뜻이므로 이 주장은
스스로 반박된다. 모델이 지목한 심볼이 카탈로그에 등록돼 있으면 구조화기의 기존 재시도 루프
(`_audience_repair_error`)를 태우게 했다 — 새 기계가 아니라 이미 있던 배선이다.

```
전: status=unsupported  (사용자에게 "표현할 수 없습니다")
후: status=success, SQL 출고
```

반대 방향도 함께 고정했다: 정말 등록되지 않은 심볼(`weather_forecast`)이면 재시도로 몰아붙이지
않는다. 목표는 미지원을 줄이는 것이 아니라 **틀린** 미지원을 없애는 것이다.

## 결과

| | 전 | 후 |
|---|---|---|
| pytest | 1,194 passed / 24 skipped | **1,270 passed / 24 skipped** |
| preflight | PASS | **PASS** |
| 커버리지 차집합 | 57 컬럼 / 19축 | **53 컬럼 / 19축** |
| 실행 자산 색인의 코퍼스 히트 | 51/77 | **74/77** |
| `required_clauses` 선언 | 5/77 | **77/77** |

**라이브 77종 전수 실측**(둘 다 API 재시작 후, 같은 러너·같은 코퍼스):
...
SQL 출고가 2배가 됐고 미지원이 12건 줄었다. 되묻기 9건 증가는 **대부분 그 12건의 행선지**다 —
'틀린 미지원'이 정직한 되묻기로 바뀐 것이고, 그것이 P2 가 노린 방향이다(§4: 목표는 미지원을
줄이는 것이 아니라 **틀린** 미지원을 없애는 것). 항목별 이동은 `--baseline` 대조로 재현된다.

**정직하게 적어 둘 것 — 두 건은 SQL 을 잃었다.** #49·#55(`사료, 간식, 장난감을 모두/하나라도
구매한 회원`)가 sql → clarification 으로 갔다. `--only 49,55 --repeat 3` 으로 갈라 본 결과
러너가 **둘 다 방출 편차로 판정**했다(반복 3회 안에서 귀결이 서로 달랐고, 3회 중 사유도
`semantic_ir_unsupported` 와 `semantic_ir_needs_clarification` 로 갈렸다). 즉 이 두 항목은
애초에 결정론적으로 SQL 을 내던 항목이 아니다 — 1회 측정의 sql → clarification 이동은
구조적 회귀의 증거가 되지 못한다.

다만 이것이 "내 변경과 무관하다"의 증명은 아니다. 증명하려면 편차 폭 자체를 재야 하고
(`--repeat` 를 양쪽에 걸어 분포를 비교), 그건 이번 라운드에서 하지 않았다.

## 함정 기록

- **API 컨테이너가 삭제된 파일을 참조하는 stale 코드로 이틀째 떠 있었다.** `/target-sql` 전건이
  `FileNotFoundError: docs/data/semantic_attribute_catalog.json` 로 500 이었는데, 그 경로 문자열은
  저장소 어디에도 없다(파일이 옮겨졌고 프로세스만 옛 모듈을 들고 있었다). 재시작 없이는 어떤
  라이브 측정도 무의미하다 — 그런데 `/health` 는 계속 200 이었다.
- **PowerShell 은 스크립트 전체를 먼저 파싱한다.** 한 줄 뒤의 파이썬 heredoc 이 파싱 오류를 내면
  **앞의 `docker restart` 도 실행되지 않는다.** 재시작했다고 믿고 30분을 엉뚱한 데서 찾았다.
  긴 인라인 스크립트는 파일로 넘기고, 재시작은 별도 호출로 확인할 것.
- **호스트 `python` 은 Windows Store 스텁이다**(출력이 `Python` 한 줄). pytest 도 라이브 러너도
  전부 `docker exec ...-python-1` 경유여야 하고, 스크립트 실행 시엔 `PYTHONPATH=/app` 이 필요하다
  (`cd /app` 만으로는 sys.path[0] 이 스크립트 디렉터리라 import 가 깨진다).
- **컨테이너마다 `/tmp` 가 다르다.** 같은 이름의 디버그 스크립트가 다른 컨테이너에 남아 있어,
  같은 코드를 돌리고 있다고 믿는 동안 서로 다른 파일이 실행됐다.
- **부분문자열 매칭은 한국어에서 조사에 걸려 죽는다.** 카탈로그 표면어 '이메일 수신 동의' 와
  실제 원문 '이메일 수신에 동의한' 은 부분문자열 관계가 아니다 — 이 방식으로는 수신동의 축
  히트가 **0**이었다. 순서보존 토큰 매칭 + 구간 길이 제한으로 바꿨다.
- **판정 함수가 no-op 인지 반드시 실측할 것.** 반증 검증이 P2 의 판정 스케치를 208개 색인으로
  돌려 히트 0 을 보였다. 그 상태로 착지했으면 P2 의 세 변경이 전부 조용히 아무 일도 안 했다.
- **로딩 시점 가드는 기존 선언을 죽일 수 있다.** 별칭 충돌을 예외로 막았더니 카탈로그가 뜨지
  않았다(소스 id 가 존재 지표로 자동 등록되므로 `campaign_response` 가 이미 임자 있는 이름이었다).
  런타임은 **정본 id 를 이기게** 두고 선언 오류는 테스트가 이름을 대는 편이 옳다.
- **문서 인용 가드는 파일을 만들기 전에 먼저 잡는다.** 도구 docstring 에 아직 없는 테스트 경로를
  적었더니 그 사이에 돌던 전체 실행이 red 였다(`test_doc_claims`).

## 남은 것

- **P1-3(`catalog_symbol_unresolved` 재방출 승격)은 하지 않았다.** 검증이 블로커 4건을 확인했다:
  `LoweringReceipt.to_dict()` 가 field/received/allowed 를 직렬화하지 않고, 재현된 실패는 합류
  분기가 아니라 **직접 lowering 분기**이며, `_merge_semantic_plan_into_event_expression` 이
  `-> None` 이라 제안된 `if merge(...): return` 이 항상 falsy 고, `build_patch_request` 는 빈 스팬
  target 을 조용히 버린다. 네 곳을 함께 고치지 않으면 배선해도 목표 시나리오에 도달하지 않는다.
- **P3 의 상대 기간 커버리지(`작년`·`어제`·`오늘`)는 하지 않았다.** 소유자가
  `calendar_window._scan_relative_calendar_months` 이고 월 grain 전용이라 grain 파라미터화가 선행돼야
  한다. 검증이 부수 결함도 하나 찾았다 — **'재작년'이 '작년'으로 잡혀 2024 가 아니라 2025 로
  해석된다**(오늘의 코드에서 이미 발생, 낱말 좌경계 부재).
- **P5 원안(라우팅 회수)은 착수하지 않았다.** 슬롯 생산자가 없어 no-op 이다. 검증이 제시한 실제
  경로는 `LLM_SEMANTIC_PLAN_NODE_TYPES`(현재 7종 중 1종만 노출 — 컴파일러 5종이 고아)를 레지스트리
  파생으로 여는 것인데, 그 상수는 import 시점 상수라 지연 함수화가 선행돼야 한다.
- **편차 폭을 재지 않았다.** #49·#55 가 불안정하다는 것은 확인했지만(러너 판정), 변경 전후의
  **분포**를 비교하지는 않았다. 1회 측정으로 두 상태를 비교하는 한 이 종류의 질문은 계속 남는다 —
  다음 라운드에서는 `--repeat` 를 양쪽에 걸고 재는 편이 낫다.
- 커밋된 실측 기준선(`live_prompt_outcomes_baseline.json`)은 **착수 직전** 수치로 남겼다.
  '후' 수치로 갱신하지 않은 이유는 위와 같다 — 편차 있는 1회 관측을 기준선으로 굳히면
  다음 회귀 판정이 그만큼 흔들린다.
