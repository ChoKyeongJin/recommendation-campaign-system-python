...
DROP_CASES = [
    ...
    # 캠페인 반응(과거 발송/접촉 성공)은 재작성기가 '캠페인'·'발송' 단어를 발송 채널로 오해해 통째로
    # 지우던 신호다 — 사라지면 소실로 잡아야 한다(원문 폴백 → 결정론 파서가 CNCT_SCS_YN 복원).
    ("캠페인에서 발송은 성공했지만 구매하지 않은 회원", "구매하지 않은 회원", ["캠페인 반응 조건 '발송/접촉 성공'"]),
    ("쿠폰을 사용한 여성 회원", "여성 회원", ["캠페인 반응 조건 '쿠폰 사용'"]),
]
...
KEEP_CASES = [
    ...
    ("발송은 성공했지만 구매하지 않은 회원", "발송 성공 후 미구매 회원"),  # 캠페인 접촉 성공 표현이 남으면 보존
]
