...
_CODE_FALLBACK: dict[str, dict[str, Any]] = {
    "vocabularies": {
        ...
        # ── 프레임(잉여어) 어휘 ────────────────────────────────────────────────────────────
        # 조건을 담지 않고 문장을 굴리기만 하는 말들. audience_frame 이 "잔여물이 조건인가"를
        # 판정할 때 쓴다. member_noun_role 을 그대로 쓰지 않고 따로 선언하는 이유는 그 어휘에
        # '구매자'가 있기 때문이다 — 프레임으로 인정하면 구매 조건이 잉여어로 위장해 통과한다.
        "audience_frame_noun": ["회원", "고객", "사용자", "유저", "고객님", "사람"],
        # 요청 지시어. 공백을 지운 문자열에 매칭하므로 낱말에 공백이 없다('찾아 줘'='찾아줘').
        "request_directive": [
            "찾아줘", "찾아주세요", "찾아주십시오", "추출해줘", "추출해주세요", "추출",
            "조회해줘", "조회해주세요", "조회", "보여줘", "보여주세요", "선정해줘", "선정해주세요",
            "뽑아줘", "뽑아주세요", "알려줘", "알려주세요", "남겨줘", "남겨주세요", "해줘", "주세요", "줘",
        ],
        # 처소격·자격 조사. 격조사(bound_particle)와 달리 조건을 만들지 않는 자리 표지다.
        # '인'은 형태상 조사가 아니라 계사 관형형이지만 하는 일이 같다('VIP인 회원').
        # '만'은 한정 보조사라 조건을 담지 않는다('VIP 회원**만**'의 조건은 VIP 쪽에 있다).
        # bound_particle 에는 넣지 않는다 — 거기는 구매 극성 정규식의 다리라서 '10만원'과 얽힌다.
        "frame_particle": ["에", "에서", "의", "로", "으로", "인", "만"],
        # 최근성 표지. 리터럴 원장은 '30일'만 바인딩하고 '최근'을 남기므로, 원장이 롤링 창임을
        # 증명한 호출자에 한해 이 낱말들이 그 리터럴의 동어반복으로 프레임에 들어간다.
        "temporal_recency_marker": ["최근", "지난", "요즘", "근래"],
        ...
        # 주 경계는 월요일 00:00 ~ 다음 월요일 00:00 전(ISO). 'N주 전'이 이미 쓰는 경계와 같다.
        "calendar_current_week": ["이번 주", "이번주", "금주"],
        "calendar_previous_week": ["지난 주", "지난주", "저번 주", "저번주", "전주"],
        "calendar_current_month": ["이번 달", "이번달", "이달", "금월", "당월"],
        "calendar_previous_month": ["지난 달", "지난달", "저번 달", "저번달", "전월"],
        ...
        "clause_scope_marker": ["중", "가운데", "중에서", "중의", "내에서"],
        ...
        # '전혀없'·'한건도없'은 '없'의 초문자열이라 단순 검색 소비자에게는 무영향이고, 인접성을
        # 요구하는 구매 극성 정규식에서만 효과가 있다. 단독형('전혀'·'한건도')을 넣으면 부정어가
        # 구매 명사 뒤에 오는 자리에서 거짓 부재가 생긴다 — 반드시 합성형으로 둔다.
        "generic_negation": [
            "없", "않", "미구매", "미구입", "미주문", "미접속", "미보유", "제외", "한번도",
            "전혀없", "한건도없",
        ],
        # '동안'·'이상의'는 기간·수량을 잇는 기능어라 값의 일부가 아니다 — 붙어 있으면
        # '동안 노트북' 처럼 어떤 상품명과도 일치할 수 없는 값이 만들어져 조건이 조용히 무력해진다.
        "event_scope_value_stopword": [
            "최근", "지난", "이번", "올해", "작년", "전체", "전부", "모든", "모두", "누적", "총", "평균",
            "첫", "최초", "재", "반복", "미", "없는", "있는", "고객", "회원", "사용자", "사람", "대상", "조건",
            "구매", "구입", "주문", "브랜드", "상품", "제품", "품목", "카테고리", "동안", "이상의",
        ],
