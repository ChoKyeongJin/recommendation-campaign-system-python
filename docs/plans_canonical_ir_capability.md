...
```
Phase 0  잔해 정리 (S~M)          — 유령 참조·죽은 표면 처분. 착수 전 필수, 즉시 가능
Phase A  ⑪ Capability 단일 권위   — ⑨(모델 A/B)와 무관하게 진행 가능. 단계별 독립 출하
Phase B  ⑩ Canonical IR 이행      — 게이트: ⑨ 결과 + B-1(=W5-1) + B-3(=W5-3②)
```
...
- A 를 B 앞에 두는 이유: A 는 선언·설정 수준이라 위험이 낮고 ⑨ 결과와 무관하게 가치가 있으며
  ...
  B 는 구조 이동이라 안전망(순수화·계약)이 먼저 필요하고, 투자 규모 자체가 ⑨ 결과에 달려 있다(NOTES.md:127).
...
- `SlotShape.resolves_unsupported`(사용 슬롯 0) 삭제.
...
## Phase B — ⑩ Canonical IR 이행

**착수 게이트 3개** (전부 충족 전 구조 이동 금지):
1. **⑨ 모델 A/B 결과**(중기 과제) — 실패 최대 군집이 "지원되는데 LLM 미방출"이므로, 상위 모델로 대부분
   해소되면 **스몰 트랙**(B-1~B-3 까지만: 경계 순수화 + 정규형 통합 + 서명 일반화), 잔존하면
   **풀 트랙**(B-4~B-5: 타입드 트리 소비 전환까지). 측정 도구는 `GOLDEN_LIVE_LLM=1` 라이브 골든 + 실패 14 코퍼스.
2. `tests/test_api_response_contract.py` green + build_sql_result 이후 `query_plan` 을 읽는 하류 지점
   전수 목록 고정(확인된 곳: `graph_rag.py:6555` 응답 조립, `:7092` capability_check).
3. Phase 0 완료(잔해 위에 경계를 긋지 않는다).
...
### B-4. adapter 경계 + freeze — 타입드 트리 shadow (L, 풀 트랙)
...
### B-5. 컴파일 소비 전환 — kind 단위 점진 (L, 풀 트랙)
