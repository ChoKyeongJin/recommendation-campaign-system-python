...
| `docs/prompts/message_generation_system.txt`       | 메시지 생성 system message       | 입력 근거 준수와 단일 variant 출력 역할을 제한                 |
...
| `message_generation_variant_user.txt` | `${query}`, `${variant}`, `${requested_channel}`, `${selected_channel_policy}`, `${campaign_context}`, `${target_context}`, `${message_examples}`, `${tone_manner_rules}`, `${repair_context}`                     |
