...
| `guides/`     | 기능별 설계/운영 가이드                            | `channel.md`, `external_conditions.md`, `ctr_analysis_process.md`, `schema_dictionary_workflow.md`, `prompt_engineering.md` |
| `data/`       | DDL, 샘플 데이터, 스키마 카탈로그, RAG 지식 베이스 | `local_bootstrap.sql`, `schema_catalog.json`, `rag_knowledge_base.json`                                                                               |
...
- `schema_extract.py`는 `--from-db`(라이브 DB, 권장)와 DDL 파일 파싱(`schema_extract.py docs/data/local_bootstrap.sql`) 두 모드를 지원한다. 접속은 `POSTGRES_*` 환경 변수를 쓰며 `--conninfo`로 재정의한다.
