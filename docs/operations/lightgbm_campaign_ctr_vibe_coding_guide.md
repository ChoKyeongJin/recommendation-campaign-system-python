...
`docs/data/ddl.sql`에는 CTR 분석 확장 테이블이 이미 포함되어 있다.
...
목표:
현재 저장소의 api.py, docs/data/ddl.sql, campaign_message_* 테이블 구조를 유지하면서
LightGBM 기반 CTR 예측 MVP를 추가한다.

현재 구조:
- FastAPI 앱은 루트 api.py에 있다.
- DB 스키마는 docs/data/ddl.sql에 있다.
