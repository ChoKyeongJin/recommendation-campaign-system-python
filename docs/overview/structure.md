...
| 조건 소유권 재조정       | `condition_reconciliation.py`                                                              | 트레이스 기반 충돌 확인요청(`conflict_clarifications`)만 잔존 — 정책 JSON(`condition_ownership_policy.json`)과 전체 재조정 파이프라인 배선은 rules 계층 철거(ac924ff)로 삭제됨 |
...
| 계층 파이프라인          | `query_pipeline/` (문서: `docs/overview/query_pipeline.md`)                                | 요구(AudienceRequirement) → 실행 사양(EventQuerySpec) → 논리 계획(LogicalPlan) → SQL(CompiledSql) 을 **서로 다른 pydantic 타입**으로 분리. 검증 전 요구는 시그니처상 컴파일러에 도달할 수 없고, 실패는 단계 이름을 달고 나온다. SQL 렌더링은 `event_compiler` 위임이라 실CRM 출력은 바이트 동일 |
| 오디언스 요구 검증/투영  | `query_structurer/audience_execution.py`, `audience_validators.py`                        | 저장 payload 의 오디언스 계약을 검증해(검증기는 요구 계층에 **주입**된다) legacy plan 키 6개(`event_expression`·`semantic_ir`·미지원 가설·실행 자산·요구 표현/issue)로 투영. 검증기 순서가 계약이다 — `semantic_ir.message` 가 첫 issue 의 문장이다 |
