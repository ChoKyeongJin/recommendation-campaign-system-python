...
사용자 입력
  ...
  ├─(2) 값·시간 한정어 정규화     semantic_normalizers   값 문법
  │                               temporal_semantics     시간 한정어 → 범용 연산자
  ...
  └─(8) 구체적 실패/clarification failure_messages       실패 종류별 한국어 렌더링
```

단계 조립은 `semantic_pipeline.run` 하나가 하고, 도메인 주입은 `semantic_plan_bridge` 하나가 한다.
...
| 파일 | 소유하는 개념 |
|---|---|
| `semantic_plan.py` | expression tree(의미 노드), 필드 선언, 결핍 계산, 실패 코드·status 닫힌 집합 |
| `temporal_semantics.py` | **범용 시간 연산자 10종**과 인자 요구, 표면 감지 실행기(어휘는 주입) |
| `semantic_normalizers.py` | 값 정규화(금액·수량·기간·랭킹·연산자·단위) |
| `semantic_capability.py` | capability 5축 판정과 실패 분류 |
| `semantic_coverage.py` | 원문 근거 대응 검증(앵커 공급자는 등록 주입) |
| `semantic_candidates.py` | 후보 병합·충돌 기록 |
| `requirement_ledger.py` | **requirement 레코드**와 귀결 회계, 회귀 판정 |
| `semantic_reemission.py` | 패치 재방출(정책·보호 집합·되돌림) |
| `semantic_pipeline.py` | 단계 조립, 컨테이너 무관 산출물 적용 |
| `semantic_plan_llm.py` | LLM 출력 계약(스키마는 노드 선언에서 파생) |
| `compile_contract.py` | `CompileContext` / `CompileResult` — 코어↔컴파일러 경계 타입 |
| `semantic_domain_binding.py` | 도메인 플러그인 바인딩(모듈 이름은 설정값) |

### 2-2. 회원 타기팅 도메인 계층

| 파일 | 소유하는 지식 |
|---|---|
| `targeting_domain.py` | 도메인 선언 로딩, **표면형 정규식 원자**, 값 어휘 파생, 코어 등록(`install()`) |
| `legacy_plan_compiler.py` | 실행 슬롯 이름과 컴파일 규칙(슬롯 지식의 단일 소유자) |
| `semantic_plan_bridge.py` | 조립 지점 — 레지스트리·컴파일러·라벨러·정책을 코어에 주입 |
| `compositional_targeting.py` | 속성 시점·이력 연산의 SQL 컴파일(검증된 슬롯만 받는다) |
| `member_attribute_history.py` | 이력 슬롯의 실행 IR 귀결 오케스트레이션 |
...
| 파일 | 선언 내용 |
|---|---|
| `docs/data/runtime/semantics/targeting_domain.json` | 노드 필드 어휘, capability 축, 실행 플랜 컨테이너, 엔터티 별칭, 계수 단위, 시간 연산자 별칭·실행 연산자 표, 조건 라벨 |
| `docs/data/runtime/semantics/semantic_capabilities.json` | 노드/지표별 지원 축, 데이터 grain, 적재 구간 |
| `docs/data/runtime/semantics/attribute_catalog.json` | 속성의 물리 바인딩, 적재 월 수 |
| `docs/data/runtime/sql/member_target_filters.json` | 속성 값 사전(등급 서열 `rank` 포함) — **값 어휘의 단일 소유자** |

---

## 3. requirement 레코드가 보존하는 것

`requirement_ledger.Requirement` 하나가 조건 하나의 전 생애를 갖는다.
...
`compiled` 일 때만 참이다. 빈 원장은 완료가 아니다.
...
`semantic_plan.FAILURE_CODES` (닫힌 집합):
...
`requirement_ledger.outcome_for` 하나이고, 응답 렌더링은
`failure_messages.requirement_failure_report` 가 `has_internal_failure` /
`has_unsupported` 로 갈라 준다.
...
| 새 의미 노드 종류 | `semantic_plan` 클래스 1개 + `semantic_capabilities.json` 선언 + 컴파일러 핸들러 |
| 새 값 종류(kind) | `semantic_plan.VALUE_KINDS` + `semantic_normalizers` 정규화기 |
| 새 실행 슬롯 | `legacy_plan_compiler` + `targeting_ir.SLOT_SHAPES` + 실행 빌더 |
...
| `tests/test_requirement_ledger.py` | 레코드 필드 누락, 실패 종류 뭉개기, 빈 원장을 완료로 읽기 |
| `tests/test_semantic_reemission.py` | 전체 재생성, 보호 집합 훼손, 정책 무시 |
| `tests/test_semantic_pipeline.py` | 가짜 성공, 내부 오류의 '미지원' 표시 |
